    ...
    if active_rows:
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                mode="markers+text",
                marker=dict(size=40, color="#10b981", opacity=0.95),
                text=[str(row["id"]) for row in active_rows],
                textfont=dict(size=20, color="#ffffff", family="Arial Black"),
                textposition="middle center",
                name="Plants",
    ...
    if idle_lat:
        fig.add_trace(
            go.Scattermapbox(
                ...
                lon=idle_lon,
                mode="markers+text",
                marker=dict(size=30, color="#94a3b8", opacity=0.9),
                text=idle_text,
    ...
    if visited_lat:
        fig.add_trace(
            go.Scattermapbox(
                ...
                lon=visited_lon,
                mode="markers+text",
                marker=dict(size=42, color="#10b981", opacity=0.95),
                text=visited_text,
...
def render_one_solution(result: Dict, active_rows: List[Dict], depot: Dict, rank: int = 1) -> None:
    ...
    with tab1:
        info_panel(
            ...
            "Blue D = depot. Green numbered circles = visited plants showing route order (1, 2, 3...). Gray markers = unvisited plants.",
        )
